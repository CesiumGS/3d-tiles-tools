import draco3d, { Attribute, Decoder, DecoderModule, Mesh } from "draco3d";

import { DracoError } from "./DracoError";
import { ComponentDatatype } from "./ComponentDataType";

// Ported from CesiumJS 'decodeDraco.js', spiced with a few lines
// from the 'draco_minimal_decoder_example.js', with some "as any"
// thrown in here and there to work around the fact that the
// typings do not match the IDL. Yes, it's a mess. Don't blame me.
export class DracoDecoder {
  ...
  decodePointCloud(properties: { [key: string]: number }, binary: Buffer) {
    const draco = this.decoderModule;
